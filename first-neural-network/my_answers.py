...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.activation_function_prime = lambda y : y * (1 - y)
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        if hidden_outputs.ndim == 1: # shape (N, ) → (N, 1)
            hidden_outputs = np.expand_dims(hidden_outputs, axis=1)
        hidden_error_term = hidden_error * self.activation_function_prime(hidden_outputs)
        
        # Weight step (input to hidden)
        delta_weights_i_h += np.outer(X, hidden_error_term.T)
...
# Goal: Lt < 0.09 and Lv < 0.18
iterations = 5000
learning_rate = 0.5611
hidden_nodes = 28
output_nodes = 1
